...
float HighTemp::getThmc(int unit = 0)
{
    float vol  = getThmcVol();

    tempThmc = K_VtoT(vol) + tempRoom;
    if(unit)
        return tempThmc * (1.8) + 32;
    else
        return tempThmc;
}
...
float HighTemp::getRoomTmp(int unit = 0)
{
...
{
    float vout = (float)getAnalog(pinThmc)/4095.0*3.3*1000;
